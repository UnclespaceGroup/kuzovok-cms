...
const { FIELD_ANNOTATION, FIELD_TEXT, FIELD_TITLE, FIELD_BANNER, FORM_ADD_REPORT } = require('../constants/WORK_FIELDS_NAME')
...
const AddServicePage = ({ location, history }) => {
  ...
  const submit = data => {
    setPending(true)
    setStatus(null)
    submitService(correctData(data), prevData.id)
      .then(res => {
        setStatus(res)
        setTimeout(() => {
          history.goBack()
